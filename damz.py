...
os.system("clear")
autoketik(f"{biru}[{kuning}Warning{biru}] {W} Virtual table Flood activated")
time.sleep(3)
os.system("clear")
autoketik(f"{biru}[{kuning}Warning{biru}] {W} Login for D@mz Flooder only private user")
time.sleep(3)
os.system("clear")
autoketik(f"""
{hijau}D@MZ PRIVATE FLOODER : Versi .1.7
{abu}-----------------------------------------
{putih}[{biru}•{Y}] {biru}Author {putih}   : Ramadhan
{putih}[{biru}•{Y}] {biru}User Name {putih}   : Analogman
{putih}[{biru}•{Y}] {biru}Title {putih}  : Analogman
{putih}[{biru}•{putih}] {kuning}Pasword {putih}: Null
{Y}[{G}•{Y}] Ip Kamu {putih}  :{Y} {ip}
{Y}[{G}•{Y}] Waktu/Jam {putih}:{Y} {localtime}
{Y}[{G}•{Y}] Total Run {putih}:{Y} {getvisit['value']}
""")

nomor = input(f"{W}[{B}• {kuning}•{hijau}•{W}] {biru}Nomor Target {W}: ")
jum = int(input(f"{W}[{B}• {kuning}•{hijau}•{W}] {merah}Jumlah {W}: "))
for i in range(jum):
    ...
    print (f"{W}[{G}✓{W}] Success Sended Flood")
    print (f"{W}[{G}✓{W}] Success Sended Flood")
